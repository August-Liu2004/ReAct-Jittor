...
class LLaMA2:
    ...
    def generate(
        ...
        for cur_pos in range(min_prompt_len, total_len):
            ...
            if stop:
             for i in range(bsz):
                if eos_reached[i]:
                    continue
                gen_tokens = tokens[i, len(prompt_tokens[i]): cur_pos + 1].tolist()
        ...
        for i, toks in enumerate(tokens.tolist()):
            ...
            if stop:
             toks = [t for t in toks if 0 <= t < self.tokenizer.n_words]
             text = self.tokenizer.decode(toks)
             min_stop_idx = min(
                [text.find(s) for s in stop if s in text] + [len(text)]
             )
             if min_stop_idx < len(text):
                toks = self.tokenizer.encode(text[:min_stop_idx],bos=False, eos=False)  
