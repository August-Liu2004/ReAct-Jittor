...
class LLaMA2Model(LLMModel):
    def __init__(self, args) -> None:
        ...
        self.generator = load(
            ckpt_dir, tokenizer_path, max_seq_len=8129, max_batch_size=1,model_name=args.model
        )
        jt.gc()
    def run(self, input_text,stop):
        ...
        with jt.no_grad():
            results = self.generator.chat_completion([dialog], max_gen_len=8129, temperature=0.6, top_p=0.9,stop=stop)
